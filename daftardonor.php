...
include('config.php');
  ...
  if (isset($_GET['nik'])) {
    ...
    $query = "SELECT * FROM tb_relawan WHERE nik='$nik'";
        ...
        <div class="row">
          ...
          <div class="col-lg-6">
            <div class="card">
              ...
              <form method="POST"action="proses/ptambah_daftar.php?nik=<?php echo $nik; ?>" enctype="multipart/form-data">
              <div class="card-body">
                <div class="form-group">
                    <label for="exampleInputEmail1">Kegiatan</label>
                    <select class="form-control select2bs4" name="id_kegiatan" style="width: 100%;">
                    <!-- <option selected disabled >Pilih Kegiatan</option> -->
                    <?php
                    $k_query = "SELECT * FROM tb_kegiatan ORDER BY id_kegiatan ASC";
                    $k_result = mysqli_query($koneksi, $k_query);
                    if(!$k_result){
                        die ("Query Error: ".mysqli_errno($koneksi).
                           " - ".mysqli_error($koneksi));
                      }                      
                      while($row=mysqli_fetch_assoc($k_result)){
                    ?>
                      <option value="<?php echo $row['id_kegiatan']; ?>">
                        <?php echo $row['hari'] ;?>, <?php echo $row['tanggal_kegiatan'] ;?>, <?php echo $row['jam'] ;?>, <?php echo $row['lokasi_kegiatan'] ;?>
                      </option>
                    <?php } ?>
                  </select>
                </div>
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="donor_ke" placeholder="">
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail1">Donor Terakhir</label>
                    <input type="date" class="form-control" name="donor_terakhir" placeholder="">
                </div>
